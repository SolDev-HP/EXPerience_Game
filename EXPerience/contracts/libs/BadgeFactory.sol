...
library BadgeFactory {
    ...
    function _prepareSVGStopPoints(uint256 tokenAmount_) private pure returns (bytes memory) {
        ...
        if (tokenAmount_ > 0 && tokenAmount_ <= 25)       { return _preparePackedColorStops(_colors[0]); }
        else if (tokenAmount_ > 25 && tokenAmount_ <= 50) { return _preparePackedColorStops(_colors[1]); }
        else if (tokenAmount_ > 51 && tokenAmount_ <= 75) { return _preparePackedColorStops(_colors[2]); }
    ...
    function _base64EncodeImage(uint256 _tokenAmount) private pure returns (string memory) {
       ...
       // string memory _svgStopColors = _prepareSVGStopColors(_tokenAmount);
        ...
        string memory _returning_svg = Base64.encode(abi.encodePacked(
                ...
                Strings.toString(_tokenAmount),            // Player's EXP balance
                '</text></svg>'                         // SVG End
            ));
